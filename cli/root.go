// Package cli assembles the kanyerest command tree from the kanyerest
...
// NewApp assembles the kit application from the kanyerest domain. The
...
//
// To add a command, declare it in kanyerest/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := kanyerest.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(kanyerest.Domain{}).Register(app)
