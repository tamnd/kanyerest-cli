...
describes what users get. Pick a format with `-o`, or let kanyerest choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kanyerest <command> -o table   # aligned columns for reading
kanyerest <command> -o jsonl   # one JSON object per line, for piping
kanyerest <command> -o json    # a single JSON array
kanyerest <command> -o csv     # spreadsheet friendly
kanyerest <command> -o tsv     # tab-separated
kanyerest <command> -o url     # just the URL column
kanyerest <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kanyerest <command> --fields id,title,url
```
...
```bash
kanyerest <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kanyerest <command>            # a table, because this is a terminal
kanyerest <command> | wc -l    # JSONL, because this is a pipe
```
